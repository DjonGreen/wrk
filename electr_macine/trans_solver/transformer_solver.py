...
class Transformer:
    """ Родительский класс трансформаторов
    """

    def __init__(self, S: float, U_gs: float, U_cs: float, f: float, Pxx: float, Ixx: float, Pk: float, Uk: float) -> None:
        """ Вводные данные трансформатора

        Args:
            S (float): Полная мощность {В*А}
            U_gs (float): Нпаряжение первичной обмотки {В}
            U_cs (float): Нпаряжение вторичной обмотки {В}
            f (float): Основная частота трансформатора {Гц}
            Pxx (float): Мощность потерь холостого хода {Вт}
            Ixx (float): Ток холостого хода {%}
            Pk (float): Мощность потерь короткого замыкания {Вт}
            Uk (float): Напряжение короткого замыкания {%}
        """
        self.S = S
        self.U_gs = U_gs
        self.U_cs = U_cs
    ...
    def get_current_gs(self) -> float:
        """ Получение тока первичной обмотки 
    ...
    def get_Rm(self) -> float:
        ...
        Returns:
            float: Сопротивление {Ом}
        """
        return self.Pxx / (self.get_current_gs()* (self.Ixx / 100))**2

    def get_Lm(self) -> float:
        """ Индуктивность магнитной цепи
    ...
    def get_R1(self) -> float:
        ...
        Returns:
            float: Сопротивление {Ом}
        """
        return self.Pk / (2 * self.get_current_gs()**2)

    def get_L1(self) -> float:
        """ Реальная индуктивность первичной омботки

        Returns:
            float: Индуктивность {Гн}
        """
        from math import pi
        return abs(((self.Uk * self.U_gs / (100 * self.get_current_gs()))**2 - (2 * self.get_R1())**2)**0.5) / (2 * pi * self.f)
    ...
    def get_k(self) -> float:
        ...
        Returns:
            float: Коэффициент
        """
        return self.U_gs / self.U_cs

    def get_current_cs(self) -> float:
        """ Ток вторичной обмотки

        Returns:
            float: Ток {А}
        """
        return self.get_current_gs() * self.get_k()
    ...
    def get_R2(self) -> float:
        ...
        Returns:
            float: Сопротивление {Ом}
        """
        return self.get_R1() / (self.get_k()**2)
    
    def get_L2(self) -> float:
        """ Реальная индуктивность вторичной обмотки

        Returns:
            float: Индуктивность {Гн}
        """
        return self.get_L1() / (self.get_k()**2)

    def get_result(self) -> str:
        """ Получение результата в текстовом виде с описанием

        Returns:
            str: Описание
        """
        return (f'\n################################'
            f'\nПараметры Трансформатора {self.S / 1000} кВА, {self.U_gs}/{self.U_cs} В\n'
